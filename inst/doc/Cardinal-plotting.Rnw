...
\end{center}
\end{figure}
Now we generate the data as if from a mass spectrometry imaging experiment with peaks at $m/z$ 3000 and $m/z$ 4000.
<<sim>>=
set.seed(1)
...
<<plot3>>=
plot(msset, pixel=pData(msset)$pg=="red", main="Mean of red pixels")
@
<<plot4>>=
...
@
\end{tabular}
\caption{\small Applying a function over pixels to plot a mean of max spectrum.}
\end{center}
\end{figure}
...
\end{center}
\end{figure}
Using a \Robject{lattice}-style formula, we can condition on other variables with \verb|image| too. Here we use all of the features, but condition on which part of the mass spectrum those features come from using the variable \verb|pData(msset)$pg|. Again, since \verb|image| knows to look in \Robject{msset}, we only need to specify the variable as \Robject{fg}.
<<image5>>=
