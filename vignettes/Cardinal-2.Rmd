...
output:
  ...
  toc: true
vignette: >
  %\VignetteIndexEntry{Cardinal 2: User guide for mass spectrometry imaging analysis}
...
The first version of *Cardinal* was written making certain assumptions about MS imaging data that are no longer true. For example, the basic assumption that the raw spectra can be fully loaded into memory is unreasonable for many MS imaging experiments today.
...
- New imaging data classes such as `ImagingExperiment`, `SparseImagingExperiment`, and `MSImagingExperiment` provide better support for out-of-memory datasets and a more flexible representation of complex experiments
...
- Support for writing imzML in addition to reading; improved support and options for importing out-of-memory imzML for both *"continuous"* and *"processed"* formats
...
- Delayed pre-processing via a new `process()` method that allows queueing of pre-processing methods such as `normalize()` and `peakPick()` for later parallel execution

- Parallel processing support via the *BiocParallel* package for all pre-processing methods and most statistical analysis methods
...
In order to specialize to the needs of different datasets, *Cardinal* provides two extensions of `MSImagingExperiment` that reflect different storage strategies for the spectral intensities.

### "Continuous" MS imaging experiments

The `MSContinuousImagingExperiment` is a specialization that enforces the data matrices be stored as dense, column-major matrices. These include R's native `matrix` and `matter_matc` objects from the *matter* package.
...
The `MSProcessedImagingExperiment` is a specialization that enforces the data matrices be stored as sparse, column-major matrices. These include `sparse_matc` objects from the *matter* package. This specialization is useful for processed data.
...
Use `selectROI()` to select regions-of-interest on an ion image. It is important to specify a subset so that selection is only made on a single experimental run. Otherwise, the form of `selectROI()` is the same as `image()`.

```{r select-ROI, eval=FALSE}
sampleA <- selectROI(mse, ms=1200, subset=run(mse) == "run0")
sampleB <- selectROI(mse, ms=1200, subset=run(mse) == "run1")
```
...
`makeFactor()` can then be used to combine multiple logical vectors (e.g., form `selectROI()`) into a single factor.
...
# get column indices corresponding to x <= 3, y <= 10 in "run0"
...
Like matrices and data frames, `rbind()` and `cbind()` can be used to combine multiple `MSImagingExperiment` objects by "row" or "column", assumping certain conditions are met.
...
Most components of an `MSImagingExperiment` that can be accessed through getter functions like `pixelData()`, `featureData()`, and `imageData()`, can also be re-assigned with analogous setter functions.

These can likewise be used to get and set columns of the pixel and feature metadata.

Note that `pData()` and `fData()` are aliases for `pixelData()` and `featureData()`, respectively.

```{r pData-set}
pData(mse)$region <- makeFactor(circle=pData(mse)$circle,
										bg=!pData(mse)$circle)
pData(mse)
```
...
*Cardinal 2* implements the verbs *dplyr* `filter()`, `select()`, `mutate()`, and `summarize()` for `MSImagingExperiment` objects.
...
Normalization to a reference is the most reliable form of normalization, but is only possible when the experiment contains a known reference peak with a constant abundance throughout the dataset. This is often not possible in unsupervised and exploratory analyses.
...
```{r peakFilter}
mse_peaks <- peakAlign(mse_pre, freq.min=0.05)
```
...
Typically, this is applied to processed profile spectra after peak detection, to extract a more accuracy representation of the peak intensities.
...
mse_bin <- mzBin(mse_pre, from=1000, to=1600, resolution=400, units="ppm")
