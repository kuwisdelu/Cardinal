...
```{r peakAlign}
mse_peaks <- peakAlign(msa_peaks, tolerance=600, units="ppm")
mse_peaks
```
...
mse_filt <- subsetFeatures(mse_peaks, freq > 0.2)
fData(mse_filt)
```

Here, we use `subsetFeatures()` to subset the data to include only peaks observed in more than 20% of the dataset. For this dataset, that results in 29 peaks. (Note that the dataset was simulated with 30 peaks.)
