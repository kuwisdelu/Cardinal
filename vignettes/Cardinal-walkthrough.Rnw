...
The open XML-based format imzML is a more recently developed format specifically designed for interchange of mass spectrometry imaging datasets \cite{Schramm}. Many other formats can be converted to imzML with the help of free applications available online.
...
\label{sec:plotIon}
Ion images from an \Robject{MSImageSet} can be plotted using the \verb|image| method. To plot the single ion image for the first feature, Figure \ref{fig:imSingleFeature}, we use:
<<image1>>=
...
In Figure \ref{fig:imMulti} the single ion images for $m/z\ 2000$, $m/z\ 3000$, and $m/z\ 4000$ are displayed simultaneously.
...
Below, we fit the first three principal components using \Rpackage{Cardinal}'s \verb|PCA| method to the TIC-normalized dataset.
...
<<pcaplot>>
@
\caption{\small PC loadings (first 3 components)}
\label{fig:pcaplot}
\end{subfigure}
\begin{subfigure}{.3\textwidth}
  \centering
...
<<pcaimage>>
@
\caption{\small PC scores (first 2 components)}
\label{fig:pcaimage}
\end{subfigure}
...
Partial least squares (PLS), also called projection to latent structures, is a multivariate method from chemometrics that has been shown to be useful for classification of mass spectrometry images. When used for classification, it is known as partial least squares discriminant analysis, or PLS-DA. PLS-DA works similarly to PCA, but it is a supervised method, so it requires labels known \textit{a priori} to train a classifier.

Here, we train a PLS classifier using the \Robject{pattern} labels from earlier.

Note that the \verb|PLS| method is vectorized on the \Robject{ncomp} argument, so multiple models will be fit, and we will specify which one we want to show when we plot the results.
...
When working with classification on real data, cross-validation should always be used, using the \verb|cvApply| method, to avoid biased results. See \verb|?cvApply|.
...
A variation on PLS, orthogonal partial least squares (O-PLS) is also implemented in \Rpackage{Cardinal}. O-PLS can often improve interpretability of the PLS model coefficients, while producing similar accuracy. See \verb|?OPLS|.



\subsection{Spatially-aware nearest shrunken centroids}

\Rpackage{Cardinal} offers a novel clustering and classification method based on the spatial smoothing of \cite{Alexandrov2011} and the nearest shrunken centroids of \cite{Tibshirani2003}. This is the \verb|spatialShrunkenCentroids| method, which can be used both for clustering and for classification.
...
Below, we do clustering on the peak-picked dataset. The parameters \Robject{r}, \Robject{k}, and \Robject{s} are the neighborhood smoothing radius, the initial number of segments, and the sparsity parameter, respectively. Since \verb|spatialShrunkenCentroids| is vectoried on these parameters, it will fit five models for the five values of \Robject{s} we have given it.
...
The \verb|spatialShrunkenCentroids| method uses statistical regularization to performed feature selection. This is shown by the "Mean \# of Features per Class" in the summary, which indicates that after \Robject{s} = 1, only three features (three peaks) are retained in the model. Since we know the entire dataset is determined by three peaks, this makes sense.
...
Rather than hard segment assignments, \verb|spatialShrunkenCentroids| assigns probabilities for pixel membership to a particular segment. \Rpackage{Cardinal} uses opacity to plot probability, with higher opacity indicating stronger probability. Since the plotted image shows high opacity in a single color for all pixels, the segmentation for this data is very high quality.
...
<<sscplot>>
@
\caption{\small Shrunken t-statistics}
\label{fig:sscplot}
\end{subfigure}
\begin{subfigure}{.3\textwidth}
  \centering
...
<<sscimage>>
@
\caption{\small Predicted segment probabilies}
\label{fig:sscimage}
\end{subfigure}
...
<<advimg1>>
@
\caption{\small Single ion image for peak at $mz500$}
\label{fig:advimg1}
\end{subfigure}
\begin{subfigure}{.3\textwidth}
  \centering
...
<<advimg2>>
@
\caption{\small Single ion image for peak at $mz510$}
\label{fig:advimg2}
\end{subfigure}
