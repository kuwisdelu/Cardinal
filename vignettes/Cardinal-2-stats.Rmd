...
Use `meansTest()` to fit linear models with the most basic summarization. The `groups` indicating the observational units must be provided. Each group is summarized by its mean, and then a linear model is fit to the summaries.
...
If an automated method for `classControl` was used, it can be helpful to plot the mapping to see what segments were used.
...
In this case, `segmentationTest()` finds one more significant mass feature compared to `meansTest()`.
