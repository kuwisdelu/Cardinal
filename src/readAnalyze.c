...
SEXP readAnalyzeSIMG(SEXP filepath, SEXP dim, SEXP dimnames)
{
	const char * filename = CHAR(STRING_ELT(filepath, 0));
	FILE * pfile = fopen(filename, "r");
	if ( pfile == NULL ) return R_NilValue;
	...
	SEXP data;
	PROTECT(data = allocMatrix(INTSXP, nrow, ncol));
	int * val = INTEGER(data);
	short int tmp[nrow];
	for ( int i = 0; i < ncol; i++ )
	{
		fread(tmp, sizeof(short int), nrow, pfile);
		for ( int j = 0; j < nrow; j++ )
		{
			val[(i * nrow) + j] = (int) tmp[j];
		}
	}
	setAttrib(data, R_DimNamesSymbol, dimnames);
	UNPROTECT(1);
	return data;
}

SEXP readAnalyzeIIMG(SEXP filepath, SEXP dim, SEXP dimnames)
{
	const char * filename = CHAR(STRING_ELT(filepath, 0));
	FILE * pfile = fopen(filename, "r");
	if ( pfile == NULL ) return R_NilValue;
	...
	for ( int i = 0; i < ncol; i++ )
	{
		fread(tmp, sizeof(int), nrow, pfile);
		for ( int j = 0; j < nrow; j++ )
		{
			val[(i * nrow) + j] = tmp[j];
		}
...
SEXP readAnalyzeFIMG(SEXP filepath, SEXP dim, SEXP dimnames)
{
	const char * filename = CHAR(STRING_ELT(filepath, 0));
	FILE * pfile = fopen(filename, "r");
	if ( pfile == NULL ) return R_NilValue;
	...
	int ncol = INTEGER(dim)[1];
	SEXP data;
	PROTECT(data = allocMatrix(REALSXP, nrow, ncol));
	double * val = INTEGER(data);
	short int tmp[nrow];
	for ( int i = 0; i < ncol; i++ )
	{
		fread(tmp, sizeof(float), nrow, pfile);
		for ( int j = 0; j < nrow; j++ )
		{
			val[(i * nrow) + j] = (double) tmp[j];
		}
	}
	setAttrib(data, R_DimNamesSymbol, dimnames);
	UNPROTECT(1);
...
SEXP readAnalyzeDIMG(SEXP filepath, SEXP dim, SEXP dimnames)
{
	const char * filename = CHAR(STRING_ELT(filepath, 0));
	FILE * pfile = fopen(filename, "r");
	if ( pfile == NULL ) return R_NilValue;
	...
	for ( int i = 0; i < ncol; i++ )
	{
		fread(tmp, sizeof(double), nrow, pfile);
		for ( int j = 0; j < nrow; j++ )
		{
			val[(i * nrow) + j] = tmp[j];
		}
